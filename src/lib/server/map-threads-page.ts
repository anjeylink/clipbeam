...
interface ThreadsPost {
  ...
  // A text post whose only content is a link to an Instagram post carries
  // that post's media here — it's what Threads plays inline in place of a
  // plain link card.
  text_post_app_info?: { linked_inline_media?: ThreadsPost | null } | null;
}
...
// Maps a post's own media, or returns null when it has none (a text post).
...
export function mapThreadsPageToMedia(html: string, shortcode: string): ResolvedMedia {
  ...
  const media = mapPostMedia(post, postUrl, authorHandle);
  ...
  const linkedMedia =
    linked && mapPostMedia(linked, postUrl, linked.user?.username ?? authorHandle);
